...
Retrieved from Scholars GeoPortal (January 17, 2021).
...
_Note: the author determined the following 7 category types. Google Map Streetview was used to spot-check a few rows to ensure their determined category fit into the assigned category type_
...
<!-- Location of family-medicine doctors (clinics and walk-ins) in the City of Hamilton were scrapped from the [College of Physicians and Surgeons of Ontario] (https://doctors.cpso.on.ca/) database in 2018. They are imported: -->
<!-- ```{r} -->
<!-- Fam_Docs_2018 <- read.csv("data-inputs/Family-Doctor-Locations/ON-Family-Doctors-Scrapped-2018.csv") %>% filter(HAM_CMA == "1") %>% select(-OBJECTID, -ID, -HAM_CMA) #family doctors outside of Hamilton CMA are filter out -->
<!-- ``` -->

<!-- Hospitals are pulled from Hamilton Open Data using GEOJSON API resource link: -->
<!-- ```{r} -->
<!-- #Hospitals: -->
<!-- Hopsitals <- geojson_read("https://opendata.arcgis.com/datasets/a5867b5375544ceb8f06544a5ed349a5_15.geojson",  what = "sp") -->
<!-- ``` -->

Licensed residential care facilities (RCFs) can also be retrieved from Hamilton Open Data but they are missing spatial geometries! They only have an address.
Import RCFs and clean:
```{r}
...
<!-- First, family-medicine doctors: -->
<!-- ```{r} -->
<!-- #rename -->
<!-- Fam_Docs_2018 <- Fam_Docs_2018 %>% rename(Supply = Sum_count) %>% mutate(Type = "Family Medicine", -->
<!--                                                                          Supply = as.character(Supply)) -->

<!-- #make header names lower case then title case -->
<!-- names(Fam_Docs_2018) <- tolower(names(Fam_Docs_2018)) %>% toTitleCase() -->

<!-- #convert to sf object -->
<!-- Fam_Docs_2018 <- Fam_Docs_2018 %>% st_as_sf(coords = c("Longitude", "Latitude"),crs = 4326) -->
<!-- ``` -->

<!-- Next, hospitals: -->
<!-- ```{r} -->
<!-- t <- geojson_read("https://opendata.arcgis.com/datasets/a5867b5375544ceb8f06544a5ed349a5_15.geojson",  what = "sp") -->

<!-- #make header names lower case then title case -->
<!-- names(t) <- tolower(names(t)) %>% toTitleCase() -->

<!-- #convert to sf object and remove unneeded columns -->
<!-- Hospitals <- st_as_sf(t) %>% st_transform(crs = 4326)  -->

<!-- Hospitals <- Hospitals %>% rename(Category = Type) %>% mutate(Type = "Hospital") %>% select(Name, Address, Category, Type) -->
<!-- ``` -->
...
<!-- Let's combine all *three* care objects we gathered so far and tag them: -->
<!-- ```{r} -->
<!-- Care_Facilities <- bind_rows(as.data.frame(Hospitals), as.data.frame(Fam_Docs_2018), as.data.frame(RCF)) %>% st_sf() -->
<!-- ``` -->

So... Recently I discovered the "Ministry of Health service provider locations" database available through the [Ontario Open Data Portal](https://geohub.lio.gov.on.ca/datasets/lio::ministry-of-health-service-provider-locations/about). Let's see what this dataset contains, when filtered to the Hamilton CMA, and compare what we retrieved from the City of Hamilton Open Data portal/scrapping the CPSO database. 
